...
comments_group_impl::comments_group_impl(access_token_function func, const std::string& file_id)
    : access_token_accessor(func),
      file_id_(file_id)
{
}

std::unique_ptr<comments_group::create_interface> comments_group_impl::create(const std::string& content,
                                                                              const std::string& fields)
{
    return std::make_unique<create_impl>(access_token_(), file_id_, content, fields);
}

std::unique_ptr<comments_group::delete_interface> comments_group_impl::del(const std::string& comment_id)
{
    return std::make_unique<delete_impl>(access_token_(), file_id_, comment_id);
}

std::unique_ptr<comments_group::get_interface> comments_group_impl::get(const std::string& comment_id,
                                                                        const std::string& fields)
{
    return std::make_unique<get_impl>(access_token_(), file_id_, comment_id, fields);
}

std::unique_ptr<comments_group::list_interface> comments_group_impl::list(const std::string& fields)
{
    return std::make_unique<list_impl>(access_token_(), file_id_, fields);
}

std::unique_ptr<comments_group::update_interface> comments_group_impl::update(const std::string& comment_id,
                                                                              const std::string& content,
                                                                              const std::string& fields)
{
    return std::make_unique<update_impl>(access_token_(), file_id_, comment_id, content, fields);
}
