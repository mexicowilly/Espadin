...
permissions_group_impl::permissions_group_impl(access_token_function func, const std::string& file_id)
    : access_token_accessor(func),
      file_id_(file_id)
{
}

std::unique_ptr<permissions_group::create_interface> permissions_group_impl::create(permission&& perm)
{
    return std::make_unique<create_impl>(access_token_(), file_id_, std::move(perm));
}

std::unique_ptr<permissions_group::delete_interface> permissions_group_impl::del(const std::string& permission_id)
{
    return std::make_unique<delete_impl>(access_token_(), file_id_, permission_id);
}

std::unique_ptr<permissions_group::get_interface> permissions_group_impl::get(const std::string& permission_id)
{
    return std::make_unique<get_impl>(access_token_(), file_id_, permission_id);
}

std::unique_ptr<permissions_group::list_interface> permissions_group_impl::list()
{
    return std::make_unique<list_impl>(access_token_(), file_id_);
}

std::unique_ptr<permissions_group::update_interface> permissions_group_impl::update(const std::string& permission_id,
                                                                               permission&& perm)
{
    return std::make_unique<update_impl>(access_token_(), file_id_, permission_id, std::move(perm));
}
